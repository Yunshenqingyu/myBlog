$(document).ready(function(){
    ...
    // 编辑文章
    $(document).on('click', '.edit-art', function(event) {
        event.stopPropagation(); // 立即阻止冒泡
        const articleId = $(this).closest('.page').data('id');
        $.ajax({
            url: `/my/article/${articleId}`,
            method: 'GET',
            headers: {
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = `/myblog/art-edit/art-edit.html?id=${articleId}`;
                } else {
                    alert('获取文章失败: ' + response.message);
                }
            },
            error: function(xhr, status, error) {
                console.error('编辑请求失败:', xhr.status, xhr.responseText);
                alert('请求失败: ' + error);
            }
        });
    });
